...
enum BattleHUDFactory {
    static func stageNode(
        ...
        root.position = CGPoint(
            x: sceneSize.width / 2,
            y: sceneSize.height - 44
        )

        let panelSize = CGSize(
            width: min(sceneSize.width - 64, 210),
            height: 54
        )
        ...
        for index in 0..<dotCount {
            let dot = SKShapeNode(
                circleOfRadius: index == currentWaveIndex ? 5.5 : 4
            )
            dot.fillColor =
                index <= currentWaveIndex
                ? .systemYellow : SKColor.white.withAlphaComponent(0.32)
            dot.strokeColor = index == currentWaveIndex ? .white : .clear
            dot.lineWidth = 1
            dot.position = CGPoint(
                x: firstX + CGFloat(index) * dotSpacing,
                y: 9
            )
            root.addChild(dot)
        }
        ...
        subtitle.text =
            config.waves[currentWaveIndex].isBossWave ? "Boss" : "Welle"
