...
struct PartnerTeamContent: View {
    ...
    var body: some View {
        if rows.isEmpty {
            CompactTeamInfo(title: "Kein Partner", message: "Wähle oder beschwöre zuerst ein Monster.")
        } else {
            VStack(spacing: 10) {
                if let active = rows.first(where: \.isSelected) ?? rows.first {
                    ActiveMonsterPanel(row: active, evolution: evolution, onEvolve: onEvolve)
                }
...
struct SupportTeamContent: View {
    ...
    var body: some View {
        if rows.isEmpty {
            CompactTeamInfo(title: "Kein Support", message: "Supporter werden aus deinen Besitzdaten angezeigt.")
        } else {
            VStack(spacing: 10) {
                ForEach(rows) { row in
                    Button {
                        ...
                        HStack(spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                Text("Lv. \(row.level)  ATK +\(percent(row.attackBonus))  DEF +\(percent(row.defenseBonus))  HP +\(percent(row.healthBonus))")
        ...
        VStack(spacing: 10) {
            HStack(spacing: 12) {
                Image(row.imageName)
                    ...
                    .clipShape(RoundedRectangle(cornerRadius: 8))
                    .overlay(RoundedRectangle(cornerRadius: 8).stroke(.cyan.opacity(0.55), lineWidth: 1))
                ...
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    StatLine(title: "Power", value: GameNumberFormatter.compact(row.power))
            ...
            if let evolution {
                Button {
                    ...
                    HStack(spacing: 8) {
                        Image(systemName: evolution.canEvolve ? "sparkles" : "lock.fill")
                        Text(evolution.canEvolve ? "Zu \(evolution.evolution.displayName) entwickeln" : "Entwicklung ab Lv. \(evolution.evolution.requiredLevel)")
                            .lineLimit(1)
                            .minimumScaleFactor(0.7)
                    }
                    .font(.system(size: 15, weight: .heavy, design: .rounded))
                    .foregroundStyle(evolution.canEvolve ? .black : .white.opacity(0.72))
                    .frame(maxWidth: .infinity)
                    .frame(height: 42)
                    .background(evolution.canEvolve ? Color.yellow : Color.gray.opacity(0.45))
                    .clipShape(RoundedRectangle(cornerRadius: 7))
                }
                .buttonStyle(.plain)
