...
struct StartView: View {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                Image("montam_logo")
                    ...
                    .animation(.easeInOut(duration: 1.5).repeatForever(autoreverses: true), value: logoPulse)
    ...
    private var startPrompt: some View {
        Text("Touch To Start")
            ...
            .padding(.vertical, 12)
            .background(
                LinearGradient(colors: [.clear, .cyan.opacity(0.62), .clear], startPoint: .leading, endPoint: .trailing)
            )
          ...
          HStack(alignment: .bottom) {
            ...
            Button {
                ...
                Text("Menü")
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 8).stroke(.black.opacity(0.75), lineWidth: 2))
...
private struct StartMenuPanel: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 12) {
                    StartMenuButton(title: "Einstellungen", icon: "gear", action: onSettings)
                    StartMenuButton(title: "Cache leeren", icon: "trash.fill", action: onCacheClear)
                    ...
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                        ForEach(AppSocialLink.all) { link in
                            StartMenuButton(title: link.title, icon: icon(for: link.id)) {
                                openURL(link.url)
                            }
                        }
                    }
    ...
    private var menuHeader: some View {
        ...
        .frame(height: 64)
        .background(
            LinearGradient(colors: [.blue, .cyan.opacity(0.75)], startPoint: .leading, endPoint: .trailing)
    ...
    var body: some View {
        Button(action: action) {
            Label(title, systemImage: icon)
                ...
                .clipShape(RoundedRectangle(cornerRadius: 8))
                .overlay(RoundedRectangle(cornerRadius: 8).stroke(.black.opacity(0.78), lineWidth: 2))
        }
        .buttonStyle(.plain)
    }
}
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 7)
                .fill(.white)
                .overlay(RoundedRectangle(cornerRadius: 7).stroke(.blue, lineWidth: 3))
        ...
        HStack(spacing: 8) {
            ForEach(0..<20, id: \.self) { index in
                RoundedRectangle(cornerRadius: 2)
                    .fill(index.isMultiple(of: 3) ? .white.opacity(0.5) : .blue.opacity(0.55))
                    .frame(width: CGFloat(6 + index % 4 * 5), height: CGFloat(6 + index % 4 * 5))
            }
        }
    }
}
